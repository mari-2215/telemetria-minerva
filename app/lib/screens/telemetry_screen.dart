...
class _TelemetryScreenState extends State<TelemetryScreen> {
  ...
  Future<void> _load({bool forceTrail = false}) async {
    ...
    try {
      ...
      final recording = widget.profile.isCaptain
          ...
          : null;
      final recordingDiscardedForInactivity = widget.profile.isCaptain &&
  ...
  String _number(Map<String, dynamic> values, String key, String suffix,
      {int decimals = 1}) {
  ...
  Future<void> _askBoatReady(Mission mission) async {
    ...
    if (confirmed != true) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text(
                  'Partida automática não autorizada. O motor permanece parado.')),
        );
      }
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            ...
            content: Text(
                'Barco confirmado como pronto. Partida automática autorizada.'),
          ),
        );
      }
    } catch (error) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text('Não foi possível autorizar a partida: $error')),
        );
      }
    }
  }
    ...
    try {
      if (_recording == null) {
        ...
        await widget.client.startRecording(
          boatId: widget.boatId,
          name: 'Trajetória DUNA ${now.day.toString().padLeft(2, '0')}-'
  ...
  Widget _motorCard(Telemetry telemetry) {
    ...
    final unauthorized =
        motorOn && _mission != null && !_mission!.startConfirmed;
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        color: color.withValues(
            alpha:
                Theme.of(context).brightness == Brightness.light ? 0.10 : 0.18),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(
                      'ESC ${_number(telemetry.propulsion, 'esc_pwm_us', 'µs', decimals: 0)} · Potência ${_number(telemetry.propulsion, 'throttle_norm', '', decimals: 2)}'),
                ],
              ),
            ),
          ],
    ...
    return Card(
      child: ListTile(
        ...
        leading: AnimatedContainer(
          ...
          child: Icon(running ? Icons.auto_mode_rounded : Icons.shield_outlined,
              color: color),
        ),
  ...
  Widget build(BuildContext context) {
    final telemetry = _telemetry;
    final recordingPoints = _recording?.points
            .map((point) => LatLng(point.latitude, point.longitude))
            .toList() ??
        const <LatLng>[];
    final missionPoints = _mission?.waypoints
            .map((point) => LatLng(point.latitude, point.longitude))
            .toList() ??
        const <LatLng>[];
                ...
                children: [
                  ...
                  GridView.count(
                    ...
                    children: [
                      _metric(
                          'Bateria',
                          _number(telemetry.power, 'battery_v', 'V'),
                          Icons.battery_charging_full_rounded),
                      _metric(
                          'Corrente',
                          _number(telemetry.power, 'current_a', 'A'),
                          Icons.electric_bolt_rounded),
                      _metric(
                          'Velocidade',
                          _number(telemetry.position, 'speed_mps', 'm/s'),
                          Icons.speed_rounded),
                      _metric(
                          'Ângulo do pod',
                          _number(
                              telemetry.propulsion, 'pod_angle_deg', 'graus'),
                          Icons.navigation_rounded),
                      _metric(
                          'Temperatura',
                          _number(telemetry.environment, 'electronics_temp_c',
                              '°C'),
                          Icons.thermostat_rounded),
                      _metric('Modo', telemetry.controlMode.toUpperCase(),
                          Icons.tune_rounded),
                    ],
                  ),
                  const SizedBox(height: 12),
                  ...
                  if (telemetry.latitude != null && telemetry.longitude != null)
                        ...
                        child: Stack(
                          children: [
                            ColoredBox(
                              ...
                              child: FlutterMap(
                                options: MapOptions(
                                  initialCenter: LatLng(telemetry.latitude!,
                                      telemetry.longitude!),
                                  initialZoom: 16,
                                ),
                                children: [
                                  ...
                                  if (missionPoints.isNotEmpty &&
                                    ...
                                    PolylineLayer(
                                      polylines: buildRoutePowerPolylines(
                                        ...
                                        cruiseThrottle:
                                            _mission!.cruiseThrottle,
                                        boatId: _mission!.boatId,
                                        strokeWidth: 6,
                                        dashed: !_mission!.startConfirmed,
                                        opacity: _mission!.startConfirmed
                                            ? 1.0
                                            : 0.82,
                                      ),
                                    ),
                                  ...
                                  MarkerLayer(
                                    markers: [
                                      ...
                                      Marker(
                                        point: LatLng(telemetry.latitude!,
                                            telemetry.longitude!),
                                        width: 58,
                                        height: 58,
                                        child: Container(
                              ...
                              Positioned(
                                ...
                                child: GestureDetector(
                                  ...
                                  child: RouteMiniMap(
                                    ...
                                    courseDeg: (telemetry.position['course_deg']
                                            as num?)
                                        ?.toDouble(),
                                  ),
                                ),
                              ),
                            if (widget.profile.isCaptain && _recording != null)
                              Positioned(
                                right: 12,
                                top: 12,
                                child: DecoratedBox(
                    ...
                    const Card(
                      child: ListTile(
                        ...
                        subtitle: Text(
                            'O mapa e a gravação aparecem assim que o NEO-6M obtiver fix.'),
                      ),
                    ),
                  const SizedBox(height: 10),
                  Text(
